...
public class ShopDaoImpl implements ShopDao{
	...
	@Override
	public String findAll(int start,int end) {
		Connection con=null;
		try {
			List<Shop> shops=new ArrayList<Shop>();
			con=DBUtils.getConnection();
			String sql="select * from (select m.*,rownum rn from (select * from shop) m) where rn between ? and ?";
			...
			List<Shop> shopx=new ArrayList<Shop>();
			...
			return "{\"total\":"+shopx.size()+",\"rows\":"+DataUtils.listToJson(shops)+"}";
		} catch (Exception e) {
			e.printStackTrace();
